...
public class 整数拆分 {
     ...
     * 时间复杂度：O(n^2)O(n
     * 2
     *  )，其中 nn 是给定的正整数。对于从 22 到 nn 的每一个整数都要计算对应的 \textit{dp}dp 值，计算一个整数对应的 \textit{dp}dp 值需要 O(n)O(n) 的时间复杂度，因此总时间复杂度是 O(n^2)O(n
     * 2
     *  )。
     *
     * 空间复杂度：O(n)O(n)，其中 nn 是给定的正整数。创建一个数组 \textit{dp}dp，其长度为 n+1n+1。
     *
     * 方法二：优化的动态规划
     * 方法一中定义的状态转移方程如下：
     *
     ...
     * 复杂度分析
     *
     * 时间复杂度：O(n)O(n)，其中 nn 是给定的正整数。和方法一相比，计算每个整数对应的 \textit{dp}dp 的值的时间复杂度降到 O(1)O(1)，因此总时间复杂度降到 O(n)O(n)。
     *
     * 空间复杂度：O(n)O(n)，其中 nn 是给定的正整数。创建一个数组 \textit{dp}dp，其长度为 n+1n+1。
     *
     * 方法三：数学
