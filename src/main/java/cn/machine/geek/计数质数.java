...
public class 计数质数 {
     ...
     *
     * 埃氏筛其实还是存在冗余的标记操作，比如对于 4545 这个数，它会同时被 3,53,5 两个数标记为合数，因此我们优化的目标是让每个合数只被标记一次，这样时间复杂度即能保证为 O(n)O(n)，这就是我们接下来要介绍的线性筛。
     *
     * 相较于埃氏筛，我们多维护一个 \textit{primes}primes 数组表示当前得到的质数集合。我们从小到大遍历，如果当前的数 xx 是质数，就将其加入 \textit{primes}primes 数组。
     *
     * 另一点与埃氏筛不同的是，「标记过程」不再仅当 xx 为质数时才进行，而是对每个整数 xx 都进行。对于整数 xx，我们不再标记其所有的倍数 x\cdot x,x\cdot (x+1),\ldotsx⋅x,x⋅(x+1),…，而是只标记质数集合中的数与 xx 相乘的数，即 x\cdot\textit{primes}_0,x\cdot\textit{primes}_1,\ldotsx⋅primes
     * 0
     * ​
     *  ,x⋅primes
     * 1
     * ​
     *  ,…，且在发现 x\bmod \textit{primes}_i=0xmodprimes
     * i
     * ​
     *  =0 的时候结束当前标记。
     *
